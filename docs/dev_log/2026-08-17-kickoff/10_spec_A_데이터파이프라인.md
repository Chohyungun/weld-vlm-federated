...
작성 2026-08-17 · 브랜치 `wt/A`
상태 **Phase B (스펙)** — 구현 코드는 §9 환경 부트스트랩 1건을 제외하고 아직 쓰지 않았다.

읽은 것: `docs/개발규약.md`, `00_cto_brief.md`, `docs/구현설계.md` 2장, `docs/실험설계.md` 3장,
...
1. **라벨 문자열을 코드에 쓰지 않는다.** `"기공"`, `"porosity"`, `"2011"` 어느 형태든 금지.
   전부 이 파일을 로드해서 얻는다. 위반은 게이트 반려 사유(`docs/개발규약.md` 불변조건 1-8).
2. **`train_class_id`는 L2에서만 부여된다.** 트랙 C가 YOLO `data.yaml`을 만들 때 자기 순서를
...
|---|---|---|---|
| `data/processed/<snapshot>/manifest.csv` | 이미지 1장 = 1행 | 70,000 | 읽기 전용 |
...
> 이 스펙의 유일한 계약 형태 변경이다. **CTO 게이트 #4(2026-08-17)에서 승인됐다.**
...
|---|---|---|
| 원본 | 이미지 + 이미지당 폴리곤 JSON | 클래스별 폴더 안의 224×224 PNG |
...
> **실측(트랙 F, 전수 sha256).** RIAWELC `testing` 2,443장은 **전량 `training` 의 바이트
> 동일 복제본**이다 — 대조 2,443/2,443 일치. 저자 분할을 그대로 쓰면 **테스트셋 전체가
> 학습셋 안에 있다. 누수율 100%다.** 원칙 문제가 아니라 이 데이터셋에서 실제로 그렇다.
>
> 트랙 A 재확인(2026-08-23, 표본 40쌍): `training` 의 같은 이름 파일이 40/40 존재하고
...
- `absolute` ↔ `conditional` ↔ `clause_only` 전환은 **CTO 승인 사항**으로 둔다. 이 값 하나가
...
- bbox 는 정수 픽셀. 학습용 좌표 변환(YOLO 정규화 / VLM 네이티브 좌표계)은 **여기서 하지 않는다.**
  트랙 C가 `to_train_coords()` / `to_orig_coords()` 한 쌍으로 처리한다(`docs/개발규약.md` 모델정책 8).
...
| 9 | 층화 허용 밴드 | **granularity 인지형** ±max(2%p, 층내 최대 묶음/층 크기), 위반 시 **기록·게이트 보고** | 고정 ±2%p + 하드 실패 | 소수 층의 ±2%p 는 묶음 원자 크기보다 좁아 정상 데이터에서도 원리적으로 실패한다. 하드 정지는 8/25 시한을 태운다 |
...
(37.5%). 상한은 게이트 #9 에서 48 → 96 으로 승인됐다.
...
| 층별(축약 라벨) 평가 비율 | 20% ± max(2%p, 층내 최대 묶음 크기 / 층 이미지 수) | 기록 + CTO 게이트 보고 후 진행 |
| 클래스별 이미지 수준 비율 (축약 전 **진짜** 클래스) | 동일 공식의 밴드 | 기록 + CTO 게이트 보고 후 진행 |
| 소수 층 최소량 | 층별 평가셋 ≥ 50장 | 리포트 지표 (게이트 판단 재료) |
...
- 모든 목표량·빈도 순서·검사 기준은 **manifest 실측에서 런타임 유도**한다. 공지 수치
  하드코딩 금지 — 승인 후 실측이 달라도 코드가 안 바뀌고, 계획 재작성 여부만 게이트가 판단한다.

## 6-10. RIAWELC 백업 경로

- 어댑터가 동일 스키마로 수렴하므로 dedup·split 코드 **무수정 완주**가 목표다. 재질 축이
  상수가 되어 층화가 클래스 단독으로 자동 퇴화한다(코드 변경 없음).
- **t\*·전처리를 이식하지 않는다** — 224×224 패치는 중복 구조가 달라 6-4 절차를 전체 재수행.
...
특히 `test_IV7_group_split_leak_detected`, `test_IV8_group_client_leak_detected`는
...
> **CTO 게이트 #4 (2026-08-17): 15건 전부 처리됐다.** Q1·Q8 은 조건부 승인(아래 표 비고),
> 나머지 13건은 기본값 승인. 표는 기록으로 남긴다.

| # | 질문 | **안전 기본값 (답 없으면 이대로 간다)** | 누가 답하나 | 언제 |
|---|---|---|---|---|
| **Q1** | 계약 #2를 `manifest.csv` 단일 파일이 아니라 `manifest.csv` + `annotations.csv` 2파일로 나눠도 되는가 | **승인(조건 2).** 하나의 SNAPSHOT.sha256 에 함께 잠금 + 조인은 §2-5 로더로만 → 둘 다 구현·테스트 완료 | CTO | ✅ 닫힘 |
| **Q2** | `verdict_mode`가 `conditional`일 때 가정 두께·스케일·품질수준 값을 무엇으로 할 것인가 | **`clause_only` 로 시작한다.** 가정값을 정하는 것은 B(허용치 CSV) 확정 이후에만 의미가 있다 | CTO + 트랙 B | 8/25 이후 |
| **Q3** | 학습 풀 안의 `val` 비율. 조기 종료를 안 쓰므로 val은 로깅 전용이다 | **클라이언트별 학습 풀의 10%, 묶음 단위·층화.** 학습에 개입하지 않는다(불변조건 3-1) | A | 게이트 |
| **Q4** | `eval_subset=judgment_2000` 표본을 언제 뽑는가 | **분할 확정과 동시에**(§6 ④ 직전) 뽑아 manifest 에 박는다. 나중에 뽑으면 채점 표본이 실행마다 달라진다 | A·D | 게이트 |
| **Q5** | RIAWELC의 `LP`(용입불량 402)는 AI허브 RT에 없다. 주 실험 라벨 공간에 미리 넣어둘 것인가 | **L2에는 정의하되 `eval_spaces.main_rt`에서 제외.** 온톨로지는 데이터셋과 독립이어야 한다 | A | 확정 |
| **Q6** | VT 74,019장을 ingest 대상에 포함할 것인가 | **manifest 에는 넣되 `modality=VT`로 표시하고 주 실험 분할에서 제외.** 확장·낯선 데이터용으로 남긴다. 지금 버리면 나중에 재수집 비용 | A | 8/25 |
| **Q7** | 스냅샷 잠금을 OS 읽기 전용 속성으로 걸 것인가, 해시 검증으로만 할 것인가 | **해시 검증 + git 추적.** Windows 읽기 전용 속성은 worktree/정션 환경에서 도구마다 다르게 동작해 오히려 사고를 낸다. 잠금 = "쓰기 시 검증기가 실패" | A | 게이트 |
| **Q8** | 트랙 C·D가 A의 manifest 확정을 기다려야 하는가 | **아니다 — 승인·완료.** `data/mock/mock_aihub_v1`(1,000행) + `mock_riawelc_v1`(300행) 커밋. §11 참조 | A | ✅ 닫힘 |
| **Q9** | `.venv` 를 worktree 5개가 각각 가질 것인가 | **각자 갖는다.** `uv sync` 로 lock 에서 재현되고 디스크 여유(226GB)가 충분하다. 공유는 트랙 간 의존성 오염 위험 | A | 확정 |
| **Q10** | AI허브 JSON 에 필름/용접부 ID 필드가 존재하는가, granularity 는 심(seam) 단위인가 | 20 ID 눈확인 통과 시 **E2 채택.** 부재·기각 시 pHash 단독으로 가고 "이동 촬영 부분 겹침 누수 잔존 가능"을 논문 한계 절에 명시 | A | 승인 직후 |
| **Q11** | 퍼콜레이션 확정 성분(서로 다른 용접부 혼입, G2 로도 미해소) 처리 | **성분 원자성 유지 + 전량 학습 풀 배정**(평가셋 금지), ST→C1 / AL→C3 고정, 건수·사유 논문 보고 | CTO | 발생 시 |
| **Q12** | 혼합 재질 묶음·교차 재질 근접쌍의 해소 방법 | **관련 이미지 전부 격리(실험 제외) + 건수 보고.** 재질 정정은 원본 불변 원칙에 따라 manifest 의 별도 override 컬럼으로만 | A·CTO | 잠금 전 필수 |
| **Q13** | 클라이언트 특정 클래스 0장을 허용할 것인가 | **허용**(의도된 non-IID). 재추첨 없음, 히트맵·클라이언트별 분해 보고 필수 | CTO | 게이트 |
| **Q14** | CLAHE 를 유지할 것인가 | **켠다.** 히스토그램 이중모드 형성 실패 시에만 CLAHE 끈 버전 1회 비교 후 총괄 에스컬레이션 | A | 6-4 실행 시 |
| **Q15** | C1 지분 수용 밴드 [0.60, 0.73] | **기본값 그대로.** 변경 시 configs 와 논문을 동시 갱신 | CTO | 게이트 |
...
**다만 남은 위험이 있다** — 아래는 아직 검증하지 않았고 트랙 C 착수 시 먼저 확인해야 한다.
...
| 2 | 계약 #2 로더·조인·잠금 (게이트 조건 1·2) | ✅ | `data/manifest_io.py` |
...
| 9 | ingest 어댑터 2벌 (RIAWELC 먼저) | ⬜ | 승인 대기와 무관하게 착수 가능 |
...
2. 조인은 `join_defects()` 만 쓴다 (게이트 조건 2).
