# 학습·연합 구현 스펙 (detection / vlm / fl)

작성 2026-08-17 · 함정 #1~#4는 다관점 병렬 검증(관점이 다른 설계안 2건을 병렬로 만든 뒤
적대 검증으로 종합)으로 작성했다.

이 문서는 스펙이다. 구현 코드는 없다. A·D 계약이 아직 얼지 않았으므로 §3의 인터페이스는
**전부 가정**이며, 어긋나는 지점을 드러내 놓는 것이 목적이다. 구현 착수 전에 맞춘다.
...
- 실행: Flower 1.x 시뮬레이션(Ray), GPU 1장. 당장은 로컬 RTX 5060 Ti 16GB(sm_120, cu128 이상).
  환경 부트스트랩(uv·Python 3.11·PyTorch cu128)은 데이터 담당 소유(함정 #5). C는 그 위에서 실측한다.
- 불변조건 전부 상속. 이 영역에서 유의할 것: 조기 종료 금지 / last 채점 / 5칸 공통 고정 8종 /
...
| 집계 | 표본 수 가중 FedAvg. 가중치는 클라이언트별 학습 표본 수에 비례한다 |
...
| 집계 | 행렬별 가중 평균 avg(A)·avg(B), 가중 n_k = 클라이언트별 D4 학습 페어 수 — **단일 소스 = B의 `counts.json`**(계약 확정 사항). **avg(B)·avg(A) ≠ avg(B·A) 논문 명시**(§4-3) |
...
성립하고 bbox 계열 경로만 비활성(§3 가정 2의 null 전파). 위치 지표 제외는 논문에 명시한다.
...
detection/
  dataset_view.py   # 계약 #2 소비 → 칸별 Ultralytics 뷰(data.yaml + 파일 리스트) 생성.
                    # **A 제공 로더 함수 경유만 — 자체 조인 금지**(계약 #2 확정 조건 2).
...
vlm/
  coords.py         # bbox 좌표 변환·역변환 단일 모듈. §4-4 처방. 평가 담당과 공유(§3 가정 9)
...
- `consumed_image_ids.txt`. 모든 학습 러너(검출·VLM·FL 클라이언트)가 실제 소비한 image_id
  목록을 run별로 남긴다. D의 누출 해시 검사(학습 소비 ∩ eval split = ∅)의 입력 .
...
| 라운드 모니터링 | **[평가셋 접근 규칙 확정, 정본 구현설계 7-4 정정 완료]** 라운드별 곡선은 **val**(학습 풀의 10%, 묶음 단위·층화, **로깅 전용** — 데이터 담당이 정의한 분할)로 낸다. 서버 evaluate 콜백은 val 기준으로 동작하고 MLflow `round`=step(클라이언트 로깅 없음). **글로벌 평가셋은 전 실험을 통틀어 최종 채점 1회만 접근한다.** 불변식은 "접근 0회"가 아니라 "평가셋이 학습·선택에 영향을 주지 않는다" — 조기 종료 금지·last 채점이라 라운드별 val 곡선에 선택압이 없다 |
| 실패 정책 | **[학습량 등가 보호 규칙 확정] `accept_failures=False` 강제** + `min_fit_clients = min_available_clients = 3` (검출·VLM 양쪽 전략). 기본값 `True`는 클라이언트 실패 시 나머지로 집계해 실효 학습량 감소가 지표에 흔적 없이 사라진다 — R×E=N을 조용히 깨는 경로. 실패 시 라운드 중단·사람 개입 |
...
**리사이즈 상·하한은 `base.yaml` 공통 고정, 프로파일 override 금지** — 계약 동결 검토에서
...
평가 담당에 통지 대상: 계약 #4 §3의 Q15 서술도 같은 정정이 필요하다.
...
> **갱신 (계약 동결 검토, 2026-08-17):** 계약 4종 동결. 아래 표의 가정 1·9·10은 게이트
> 판정으로 **확정**됐고(01_cto_gate §5-1·§5-3·계약 #2 확정), 이후 변경은 변경 승인 사항이다.

| # | 상대 | 가정 | 어긋나면 |
|---|---|---|---|
| 1 | A | **확정(계약 #2 확정):** 계약 #2는 `manifest.csv`(이미지 단위) + `annotations.csv`(결함 인스턴스 단위) **2파일, 단일 `SNAPSHOT.sha256`으로 함께 잠금.** 조인은 **A 제공 로더 함수로만** — C의 `dataset_view.py`는 자체 조인 금지, A 로더 경유 | (동결) |
...
| 4 | A | **확정(계약 확정 사항):** 두께·스케일 가용성은 **스냅샷별 `data_capabilities.yaml`**(A 구현 채택 — mock 2벌에 실물 존재). C는 이 파일을 읽는다. 종전 가정(`configs/data_flags.yaml`) 폐기 | (동결) |
...
| 7b | B | **확정(계약 확정 사항):** 클라이언트별 D4 학습 페어 수의 단일 소스는 B의 **`counts.json`**(정식 산출물). C의 통합·연합 n_k와 §6 Q6 스텝 환산이 이 파일을 읽는다 | (동결) |
| 8 | B | D3 corpus jsonl은 messages 또는 prompt/response 형식 — TRL 표준 로더로 흡수 가능 | 변환 어댑터 1개 추가 |
| 9 | D | **확정(§5-1 + 계약 확정 사항 어휘 통일):** 계약 #4는 `bbox_px`(역변환 완료 원본 픽셀) + `coord_space`·`coord_cfg_hash` 메타. **네이티브 좌표 필드 없음.** C 추가 요청 필드 `cell`·`seed`는 D 스키마에 이미 존재 확인, `raw_text`는 D의 `raw_output_ref`(파일:줄 참조) 방식 수용 | D의 §5-1 반영 재보고로 동결 발효 |
| 10 | D | **확정(§5-1): C = 규약 소유자, D = 검증 소유자.** `vlm/coords.py`는 학습 담당 소유·**의존성 제로 리프 모듈 준수 의무**·D가 import. D의 검증 장치 전부 유지: 골든 픽스처 독립 재실행 + Q20 실측 봉인(Ultralytics xyxy 원본 좌표 가정, 표본 50장) + D1 정규화 잔재 검사 + 해시 대조 | (동결) |
...
| 12 | D | MLflow 규약(평가 담당 소유): experiment `weld-fl`, run 명명 `{cell}_{seed}_{yymmdd}`, FL은 round=step. C는 이 규약대로 로깅만 | 규약 확정본을 따라감 |
| 13 | D | latency·VRAM 측정 스크립트는 평가 담당 소유. C는 추론 진입점(`export_preds.py`, `infer_eval.py`)을 batch=1 호출 가능하게 제공 | — |
...
> 문장·출처·기각 사유)은 `12a_함정판정_다관점검증_C.md`** 에 있다.
...
- **EMA: 검출 3칸 공통 비활성. raw fp32 가중치를 교환·집계·채점.** stock last.pt(EMA
  ...
  깨지므로 비활성이 유일한 대칭 해법.
  **[계약 동결 검토 확인 요청에 대한 답: 확정]** FedAvg가 평균하는 것은 **raw weight다.
...
- **평가 접근 정책 [평가셋 접근 규칙로 통일]:** 라운드별 곡선은 **val**(학습 풀 10%, 로깅
...
- 메인 런은 **관측 전용 진단**만: 누락 분산비 ρ(가중 평균 분산이 놓치는 클라이언트 간 평균
  ...
  평가셋 접근 규칙에 따라 12a 함정 #2 열린질문 6의 "글로벌 평가셋 evaluate 확장" 기본값은 이 기준으로
...
- VLM 라운드 곡선은 **val loss**(teacher-forcing, 생성 없음) 기준 서버 evaluate(계약 확정 사항
  평가셋 접근 규칙). 생성 기반 지표는 최종 채점 1회: 곡선이 더 필요하면 마일스톤 어댑터 사후 산출.
...
어휘는 **`coord_space`로 통일**(계약 확정 사항: 종전 표기 `convention` 폐기).

- **저장 계층은 원본 픽셀 단일.** 모델 좌표는 어떤 파일에도 저장 금지. 변환은 학습 담당 소유
...
- **평가 담당은 같은 모듈을 import하고 C의 골든 픽스처를 재실행** — 채점기 자체는 bbox_px만
...
- 공통 프로토콜: 칸별 대표 구성으로 **100 step 파일럿** → `torch.cuda.max_memory_allocated`,
  step 시간 기록 → GPU-일 추정치 갱신(리스크 등록부의 18~27 GPU-일 재추정 입력).
- OOM 시 조정 사다리 **[리사이즈 상·하한 동결 규칙로 재작성: `max_pixels`는 어떤 단계에서도 손대지
  않는다(Q15 동결)]**:
  검출 = 배치 크기↓ (해상도 640 유지) /
  VLM = 배치 크기↓ → gradient accumulation↑(유효 배치 유지) → gradient checkpointing
  (기본 켬 — §2-4) → **(최후) 모델 크기** 재검토(재검토 안건). 그래도 안 되면 48GB 이관.
...
한 줄 표를 파일럿 보고로 파일럿 보고에 싣는다.

---

# 6. 열린 질문 (전부 안전 기본값 — 확정 전까지 이 값으로 진행)

다관점 병렬 검증 판정으로 Q1~Q3은 기본값이 **확정안으로 승격**됐다(확정 대기). 함정별 세부
...
| Q3 | FL 라운드 모니터링의 평가 데이터 | **닫힘(평가셋 접근 규칙, 정본 7-4 정정 완료):** 라운드별 곡선 = **val**(학습 풀 10%, 묶음 단위·층화, 로깅 전용). 글로벌 평가셋 = 최종 채점 1회. 구현설계 3-3(BN 재추정) 정정 제안은 §4-2대로 유지(별도 안건) |
